...
const loginUser = async (req, res) => {
    ...
    return res
        ...
        .json(
            new ApiResponse(
                200,
                { role },   // 👈 THIS IS THE ROLE FRONTEND USES
                "Login successful"
            )
        );
};
...
const refreshAccessToken = async (req, res) => {
    ...
    let user, decoded, role;

    try {
        decoded = jwt.verify(token, process.env.STUDENT_REFRESH_TOKEN_SECRET);
    ...
    if (!user) {
        try {
            decoded = jwt.verify(token, process.env.HR_REFRESH_TOKEN_SECRET);
    ...
    if (!user) {
        try {
            decoded = jwt.verify(token, process.env.INSTITUTE_REFRESH_TOKEN_SECRET);
    ...
    return res
        .cookie("accessToken", newAccessToken, { httpOnly: true, sameSite: "lax" })
